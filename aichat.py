...
def main(device):
    IMG_WIDTH = 512
    input_image, extra_image = prepare_input_img(IMG_WIDTH, args.character)
    ...
    model_output = None
    model_process = ModelClientProcess(input_image, device)
    ...
    while True:
        ...
        idle_flag = False
        # if bool(ai_process.model_process.is_speech.value):  # time.time() % 24 < 16
        #     if not ai_process.model_process.speech_q.empty():
        #         speech_q = ai_process.model_process.speech_q.get_nowait()
        #     eyebrow_vector_c, mouth_eye_vector_c, pose_vector_c = action.speaking(speech_q)
        # elif bool(ai_process.model_process.is_singing.value):  # time.time() % 24 < 16
        #     if not ai_process.model_process.beat_q.empty():
        #         beat_q = ai_process.model_process.beat_q.get_nowait()
        #     if not ai_process.model_process.mouth_q.empty():
        #         mouth_q = ai_process.model_process.mouth_q.get_nowait()
